// Original file: <umbraco src>\src\Umbraco.Web.UI.Client\src\views\propertyeditors\contentpicker\contentpicker.controller.js
function danfossMultinodeTreePickerController($scope, entityResource, editorState, iconHelper, $routeParams, angularHelper, navigationService, $location, localizationService, editorService, $q) {
    ...
    // Danfoss customization
    // Perform multi or single picker check. Originally implemented in MultiNodeTreePicker2PropertyEditor class, method ConvertDbToEditor
    ...
    if (entityType === "Document") {
        if ($scope.model.config.displaySelected) {
            var danfDocTypeFilter = null;
            // Configure filtering logic. Default functionality of TreePickerController
            if (dialogOptions.filter) {
                // save filter specified before
                dialogOptions.danfFilter = dialogOptions.filter;
                dialogOptions.danfFilterExclude = false;

                //used advanced filtering
                if (!angular.isFunction(dialogOptions.danfFilter) && !angular.isObject(dialogOptions.danfFilter)) {
                    if (dialogOptions.danfFilter.startsWith("!")) {
                        dialogOptions.danfFilterExclude = true;
                        dialogOptions.danfFilter = dialogOptions.danfFilter.substring(1);
                    }

                    // save filter of DocumentType, originally placed in performFiltering of TreePickerController (!filterAdvanced).
                    danfDocTypeFilter = dialogOptions.danfFilter.toLowerCase().replace(/\s/g, '').split(',');
                }
            }

            dialogOptions.filter = function (item) {
                _.each($scope.renderModel, function (added) {
                    var matched = $scope.model.config.idType === "udi"
                        ? added.udi == item.udi
                        : added.id == item.id;
                    if (matched) {
                        // display item as selected
                        // item.selected = true;
                        item.cssClasses.push("umb-tree-node-checked");
                        // disable click handlers on item
                        item.filtered = true;
                    }
                });

                // perform original document type filtering of TreePickerController
                if (danfDocTypeFilter) {
                    var found = danfDocTypeFilter.indexOf(item.metaData.contentType.toLowerCase()) >= 0;

                    if (!dialogOptions.danfFilterExclude && !found || dialogOptions.danfFilterExclude && found) {
                        item.filtered = true;
                        ...
                        if (dialogOptions.filterCssClass) {
                            ...
                            item.cssClasses.push(dialogOptions.filterCssClass);
                        }
                    }
                }
...
angular.module('umbraco').controller("Danfoss.MultinodeTreePickerController", danfossMultinodeTreePickerController);